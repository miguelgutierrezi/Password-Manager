	...
	public AddAccountView(final Usuario u) {
		...
		btnAadirCuenta.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if ((textFieldNombre.getText().length() != 0) && (textFieldUsuario.getText().length() != 0)
						&& (textFieldURL.getText().length() != 0) && (passwordField.getPassword().length != 0)) {
					...
					if ((passwordField.getPassword().length < 8)
						...
						try {
							password = Utils.encrypt(key, iv, new String(passwordField.getPassword()));
						} catch (Exception e1) {
